...
!---------------------------------------------------------------------------------------------------------
  ...
  subroutine readland_hrldas(wrfinput_flnm,                            &
    ...
    ! read slope of topography
    if (present(slopert)) then
    ...
    if (present(ielev)) then 
        call get_2d_netcdf("ielev", ncid, ielev, units, xstart, xend, ystart, yend, FATAL, ierr)
        if (ierr /= 0) print*, 'Did not find ielev, only need for iopt_run == 9/10'
    endif

    ! read acquifer poros, [-] 
    if (present(gwporos)) then
        call get_2d_netcdf("gwporos", ncid, gwporos, units, xstart, xend, ystart, yend, FATAL, ierr)
...
!---------------------------------------------------------------------------------------------------------
  ...
  subroutine get_2d_netcdf(name, ncid, array, units, xstart, xend, ystart, yend, &
    ...
    iret = nf90_inq_varid(ncid,  name,  varid)
    if (iret /= 0) then
       iret = nf90_inq_varid(ncid, name//"_M",  varid)
...
!     For dealing with per variable chunk cache.
    ...
    if (output_count_remember == 0) then
       ...
       if (mod(output_timestep,3600) == 0) then
          write(output_flnm, '(A,"/",A10,"_LDASOUT_DOMAIN",I1)') outdir, date(1:4)//date(6:7)//date(9:10)//date(12:13), igrid
       elseif (mod(output_timestep,60) == 0) then
          write(output_flnm, '(A,"/",A12,"_LDASOUT_DOMAIN",I1)') outdir, date(1:4)//date(6:7)//date(9:10)//date(12:13)//date(15:16), igrid
       else
          write(output_flnm, '(A,"/",A14,"_LDASOUT_DOMAIN",I1)') outdir, date(1:4)//date(6:7)//date(9:10)//date(12:13)//date(15:16)//date(18:19), igrid
       endif
